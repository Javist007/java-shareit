...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponse findById(long bookingId, long userId) {
        ...
        if (booking.getBooker().getId() != userId &&
            ...
            log.warn("Пользователь {} попытался получить доступ к бронированию {}, но нет прав",
                    userId, bookingId);
            throw new ForbiddenException("Недостаточно прав");
        }

        log.debug("Бронирование найдено: {}", booking);
    ...
    public BookingResponse create(BookingDto request, long userId) {
        ...
        if (!item.getAvailable()) {
            log.warn("Запрос на бронирование недоступного товара: itemId={}", request.getItemId());
        ...
        if (request.getEnd().isBefore(request.getStart())
            ...
            log.warn("Неверные даты: start={}, end={}", request.getStart(), request.getEnd());
            throw new ValidationException("Дата окончания должна быть после даты начала");
        }

        if (item.getOwner().getId() == userId) {
            log.warn("Пользователь id={} пытается забронировать собственный товар", userId);
        ...
        if (isOverlapping) {
            log.warn("Пересекающее бронирование для itemId={}", request.getItemId());
    ...
    public BookingResponse approve(long bookingId, Boolean approved, long userId) {
        if (approved == null) {
            log.warn("'approved'==null при попытке изменить статус бронирования");
        ...
        if (booking.getItem().getOwner().getId() != userId) {
            log.warn("Пользователь {} пытается подтвердить бронирование {}, не своего товара",
                    userId, bookingId);
            throw new ForbiddenException("Только владелец может подтверждать");
        }

        if (booking.getStatus() != BookingStatus.WAITING) {
            log.warn("Бронирование id={} находится в статусе {}, а не WAITING",
    ...
    public List<BookingResponse> findAllByBooker(BookingState state, long userId) {
        if (!userRepository.existsById(userId)) {
            log.warn("Пользователь id={} не найден при поиске бронирований", userId);
    ...
    public List<BookingResponse> findAllByOwner(BookingState state, long userId) {
        if (!userRepository.existsById(userId)) {
            log.warn("Пользователь id={} не найден при поиске бронирований владельца", userId);
            throw new NotFoundException("Пользователь не найден");
        }
