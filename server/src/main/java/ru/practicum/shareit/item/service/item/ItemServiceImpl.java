...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemResponse addItem(ItemDto request, long ownerId) {
        ...
        if (request.getRequestId() != null) {
            itemRequest = itemRequestRepository.findById(request.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос товара не найден"));
        }

        Item item = ItemMapper.toEntity(request, itemRequest);
        item.setOwner(owner);

        log.debug("Вещь сохранена: ownerId={}, name={}", ownerId, request.getName());
    ...
    public ItemResponse updateItem(long itemId, ItemDto request, long ownerId) {
        ...
        if (existing.getOwner().getId() != ownerId) {
            log.warn("Пользователь {} попытался обновить чужую вещь {}", ownerId, itemId);
    ...
    public ItemResponse getItemById(long id, long userId) {
        ...
        log.debug("Вещь {}: owner={}, comments={}", id, item.getOwner().getId(), comments.size());
    ...
    public List<ItemResponse> findAllOwnerItems(long ownerId) {
        ...
        if (items.isEmpty()) {
            log.debug("Вещи для владельца {} отсутствуют", ownerId);
            return List.of();
        }
        ...
        log.debug("Найдено {} вещей для владельца {}", items.size(), ownerId);
        return items.stream()
    ...
    public List<ItemResponse> search(String text) {
        if (text == null || text.isBlank()) {
            log.debug("Пустой запрос – возвращаем пустой список");
            return List.of();
        }
        List<Item> found = itemRepository.search(text);
        log.debug("Найдено {} элементов", found.size());
